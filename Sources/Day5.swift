struct Day5: Solution {
    ...
    func test() {
        ...
        assert(
            calculate(
                updates: updatesNotInRightOrder.map { $0.putInRightOrder(rules: rules) },
                rules: rules) == 123, "expected exactly 123")
    }
    ...
    func run(_ input: String) {
        ...
        print(
            "Result for updates not in order = \(calculate(updates: updates.filterNotInRightOrdered(rules: rules).map { $0.putInRightOrder(rules: rules) }, rules: rules))"
        )
    }

    private func calculate(updates: [[Int]], rules: [Int: [Int]]) -> Int {
    ...
    private func parseUpdates(_ input: String) -> [[Int]] {
        ...
        while result.isNotInRightOrder(withRules: rules) {
            for i in 0..<result.count - 1 {
                if !rules[result[i], default: []].contains(result[i + 1]) {
                    ...
                    if let rulesIdx = rules.firstIndex(where: {
                        $0.key == result[i + 1] && $0.value.contains(result[i])
                    }) {
